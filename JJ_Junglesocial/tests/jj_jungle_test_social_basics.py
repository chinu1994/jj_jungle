...
class TestSocialBasics(common.SocialCase, CronMixinCase):
    ...
    def test_cron_triggers(self):
        ...
        with self.capture_triggers('social.ir_cron_post_scheduled') as captured_triggers:
            social_post = self.env['jj.social.post'].create({
    ...
    def test_social_account_internals(self):
        ...
        accounts = self.env['jj.social.account'].create(vals_list)
    ...
    def test_social_post_create_multi(self):
        """ Ensure that a 'multi' creation of 2 jj.social.posts also
        creates 2 associated utm.sources. """
        social_posts = self.env['jj.social.post'].create([{
            'account_ids': [(4, self.social_account.id)],
            'message': 'Message 1'
        }, {
            'account_ids': [(4, self.social_account.id)],
    ...
    def test_social_post_utm(self):
        ...
        post_1, post_2, post_3, post_4, post_5, post_6, post_7 = self.env['jj.social.post'].create([{
            'account_ids': [(4, self.social_account.id)],
            'message': 'Message 1',
        }, {
            'account_ids': [(4, self.social_account.id)],
    ...
    def test_social_post_create_with_default_calendar_date(self):
        ...
        See jj.social.post#create for more details."""
        form = Form(self.env['jj.social.post'].with_context(default_calendar_date='2022-05-29 05:00:00'))
