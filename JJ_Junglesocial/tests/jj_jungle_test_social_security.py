...
class TestAccess(common.SocialCase):
    ...
    def setUpClass(cls):
        ...
        with mock_void_external_calls():
            cls.jj_social_live_post = cls.env['jj.social.live.post'].create({
                'post_id': cls.social_post.id,
                'account_id': cls.social_account.id
            })

            cls.social_stream_type = cls.env['jj.social.stream.type'].create({
                'name': 'My Stream Type',
                'stream_type': 'my_stream_type',
                'media_id': cls.social_media.id
            })

            cls.social_stream = cls.env['jj.social.stream'].create({
                'account_id': cls.social_account.id,
                'stream_type_id': cls.social_stream_type.id,
                'media_id': cls.social_media.id
            })

            cls.social_stream_post = cls.env['jj.social.stream.post'].create({
                'message': 'A stream post',
                'stream_id': cls.social_stream.id,
            })

            cls.social_stream_post_image = cls.env['jj.social.stream.post.image'].create({
    ...
    def test_access_social_employee(self):
        ...
        with self.assertRaises(AccessError):
            self.env['jj.social.post'].create({'message': 'A Post'})
        with self.assertRaises(AccessError):
            self.env['jj.social.live.post'].create({'post_id': self.social_post.id, 'account_id': self.social_account.id})
        with self.assertRaises(AccessError):
            self.env['jj.social.account'].create({'name': 'An account', 'media_id': self.social_media.id})
        with self.assertRaises(AccessError):
            self.env['jj.social.media'].create({'name': 'A media'})
        with self.assertRaises(AccessError):
            self.env['jj.social.stream'].create({'name': 'A stream', 'account_id': self.social_account.id})
        with self.assertRaises(AccessError):
            self.env['jj.social.stream.type'].create({'name': 'A stream type'})
        with self.assertRaises(AccessError):
            self.env['jj.social.stream.post'].create({'message': 'A stream post'})
        with self.assertRaises(AccessError):
            self.env['jj.social.stream.post.image'].create({'image_url': 'dummy.png'})

        # Read: not allowed
        with self.assertRaises(AccessError):
            self.social_post.with_user(self.env.user).read(['message'])
        with self.assertRaises(AccessError):
            self.jj_social_live_post.with_user(self.env.user).read(['post_id'])
        with self.assertRaises(AccessError):
        ...
        with self.assertRaises(AccessError):
            self.jj_social_live_post.with_user(self.env.user).write({'post_id': self.social_post.id})
        with self.assertRaises(AccessError):
        ...
        with self.assertRaises(AccessError):
            self.jj_social_live_post.with_user(self.env.user).unlink()
        with self.assertRaises(AccessError):
    ...
    def test_access_social_social_user(self):
        ...
        new_post = self.env['jj.social.post'].create({'message': 'A Post'})
        new_stream = self.env['jj.social.stream'].create({
        ...
        self.env['jj.social.live.post'].create({
            'post_id': self.social_post.id,
            'account_id': self.social_account.id
        })
        new_live_post = self.env['jj.social.live.post'].create({
            'post_id': new_post.id,
            'account_id': self.social_account.id
        })
    ...
    def test_access_social_social_manager(self):
        ...
        new_post = self.env['jj.social.post'].create({'message': 'A Post', 'account_ids': [(4, self.social_account.id)]})
        self.env['jj.social.live.post'].create({'post_id': self.social_post.id, 'account_id': self.social_account.id})
        self.env['jj.social.account'].create({'name': 'An account', 'media_id': self.social_media.id})
        self.env['jj.social.stream'].create({
            'media_id': self.social_media.id,
            'stream_type_id': self.social_stream_type.id,
            'account_id': self.social_account.id
        })
        self.env['jj.social.stream.post'].create({'message': 'A stream post', 'stream_id': self.social_stream.id})
        self.env['jj.social.stream.post.image'].create({'image_url': 'dummy.png'})
    ...
    def _get_social_media(cls):
        return cls.env['jj.social.media'].create({
            'name': 'Social Media',
        })
