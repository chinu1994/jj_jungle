...
class TestUTMConsistencySocial(TestUTMCommon, SocialCase):
    ...
    def setUpClass(cls):
        ...
        cls.user_social_manager = cls.env['res.users'].create({
            ...
            'groups_id': [(6, 0, [cls.env.ref('JJ_Junglesocial.group_social_manager').id])],
        })
    ...
    def test_utm_consistency_social_account(self):
        ...
        with self.assertRaises(UserError):
            # can't unlink the medium as it's used by a jj.social.post as it's medium
    ...
    def test_utm_consistency_social_post(self):
        ...
        with self.assertRaises(UserError):
            # can't unlink the source as it's used by a jj.social.post as its source
