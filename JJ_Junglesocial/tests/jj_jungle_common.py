...
class SocialCase(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        with mock_void_external_calls():
            ...
            cls.social_post = cls.env['jj.social.post'].create({
                'message': 'A message',
            ...
            cls.social_manager = mail_new_test_user(
                ...
                groups='JJ_Junglesocial.group_social_manager,base.group_user'
            )

            cls.social_user = mail_new_test_user(
                cls.env, name='Lukas Peeters', login='social_user', email='social.user@example.com',
                groups='JJ_Junglesocial.group_social_user,base.group_user'
            )
    ...
    def _get_social_account(cls):
        return cls.env['jj.social.account'].create({
    ...
    def _get_post_social_accounts(cls):
        return cls.social_account | cls.env['jj.social.account'].create({
            'media_id': cls._get_social_media().id,
            'name': 'Social Account 2'
        })

    def _checkPostedStatus(self, success):
        live_posts = self.env['jj.social.live.post'].search([('post_id', '=', self.social_post.id)])
