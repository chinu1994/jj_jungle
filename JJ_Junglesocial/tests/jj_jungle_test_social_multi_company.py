...
class TestSocialMultiCompany(common.SocialCase):
    ...
    def setUpClass(cls):
        ...
        with mock_void_external_calls():
            cls.media_id = cls.env['jj.social.media'].create({'name': 'Social Media'})
            ...
            cls.account_3 = cls.env['jj.social.account'].create({
                'name': 'Account 3',
                'media_id': cls.media_id.id,
                'company_id': False,
            })

            cls.stream_type_id = cls.env['jj.social.stream.type'].create({
                'name': 'Stream Type',
                'stream_type': 'stream_type',
                'media_id': cls.media_id.id,
            })

            cls.stream_1 = cls.env['jj.social.stream'].create({
                'name': 'Stream 1',
                'media_id': cls.media_id.id,
                'account_id': cls.account_1.id,
                'stream_type_id': cls.stream_type_id.id,
            })
            cls.stream_2 = cls.env['jj.social.stream'].create({
                'name': 'Stream 2',
                'media_id': cls.media_id.id,
            ...
            cls.stream_post_1 = cls.env['jj.social.stream.post'].create({
                'stream_id': cls.stream_1.id,
            })

            cls.stream_post_2 = cls.env['jj.social.stream.post'].create({
    ...
    def _get_social_media(cls):
        return cls.env['jj.social.media'].create({'name': 'Social Media'})
    ...
    def test_social_account_acls(self):
        ...
        with self.assertRaises(AccessError, msg='Should not be able to read the account of an other company'):
            self.env['jj.social.account'].browse(self.account_2.id).name
    ...
    def test_social_post_acls(self):
        ...
        post = self.env['jj.social.post'].create({
    ...
    def test_social_post_click_count(self):
        ...
        url = 'https://jj_jungle.com/test/click/count/computation'
        message = f"""
            Hi social users :)
            Visit {url}
        """
    ...
    def test_social_stream_post_acls(self):
        ...
        result = self.env['jj.social.stream.post'].search([])
        self.assertEqual(self.stream_post_1, result)

        self.env.invalidate_all()

        result = self.env['jj.social.stream.post'].with_user(self.social_manager).search([])
