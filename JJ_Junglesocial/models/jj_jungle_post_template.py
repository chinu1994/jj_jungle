...
class JJSocialPostTemplate(models.Model):
    """
    Models the abstraction of social post content.
    It can generate multiple 'jj.social.post' records to be sent on social medias
    ...
    'jj.social.post.template' is therefore a template model used to generate `jj.social.post`.
    It is inherited by `jj.social.post` to extract common fields declaration and post
    management methods.
    """
    _name = 'jj.social.post.template'
    _description = 'JJ Social Post Template'
    _rec_name = 'message'

    @api.model
    def default_get(self, fields):
        result = super(JJSocialPostTemplate, self).default_get(fields)
    ...
    def _jj_prepare_post_content(self, message, media_type, **kw):
        ...
        if media_type not in [key for (key, val) in self.env['jj.social.media'].fields_get(['media_type'])['media_type']['selection']]:
