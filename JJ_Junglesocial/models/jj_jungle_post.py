...
class JJSocialPost(models.Model):
    """ A jj.social.post represents a post that will be published on multiple jj.social.accounts at once.
    ...
    _name = 'jj.social.post'
    _inherit = ['mail.thread', 'mail.activity.mixin', 'jj.social.post.template', 'utm.source.mixin']
    _description = 'jj Social Post'
    ...
    def _jj_compute_account_allowed_ids(self):
        ...
        social account.
        """
        all_account_allowed_ids = self.env['jj.social.account'].search([])
    ...
    def default_get(self, fields):
        ...
        result = super(JJSocialPost, self).default_get(fields)
    ...
    def create(self, vals_list):
        ...
        res = super(JJSocialPost, self).create(vals_list)
    ...
    def _action_post(self):
        """ Called when the post is published on its jj.social.accounts.
        It will create one jj.social.live.post per jj.social.account and call '_post' on each of them. """
        ...
        for post in self:
            # send the live posts
            failed_posts = self.env['jj.social.live.post']
