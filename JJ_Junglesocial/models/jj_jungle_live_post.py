...
class JjSocialLivePost(models.Model):
    """ A social 'live' post, as opposed to a jj.social.post, represents a post that is
    actually on a jj.social.account instance.

    Basically, a jj.social.post that is posted on 4 jj.social.accounts will create 4 instances
    of the jj.social.live.post. """

    _name = 'jj.social.live.post'
    _description = 'JJ Social Live Post'

    post_id = fields.Many2one('jj.social.post', string="Social Post", required=True, readonly=True, ondelete="cascade")
    account_id = fields.Many2one('jj.social.account', string="Social Account", required=True, readonly=True, ondelete="cascade")
    ...
    state = fields.Selection([
        ...
        help="""Most jj.social.live.posts directly go from Ready to Posted/Failed since they result of a single call to the third party API.
    ...
    @api.depends(lambda self:
        ...
        ['post_id.%s' % field for field in self.env['jj.social.post']._get_post_message_modifying_fields()])
    ...
    def name_get(self):
        """ ex: [Facebook] JJ_Jungle  Social: posted, [Twitter] Mitchell Admin: failed, ... """
    ...
    def _refresh_statistics(self):
        ...
        It will be called manually every time we need to refresh the jj.social.stream data:
            - jj.social.stream creation/edition
            - 'JJ Jungle Feed' kanban loading
