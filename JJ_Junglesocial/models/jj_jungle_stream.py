...
class JJSocialStream(models.Model):
    """jj.social.streams are used to fill the 'Feed' view that allows users to follow the jj.social.media activity
    based on their interest (a Facebook Page, a Twitter hashtag, ...).

    They use the jj.social.media third party API to fetch the stream data and create jj.social.stream.posts
    that are displayed on the Feed kanban view. """

    _name = 'jj.social.stream'
    _description = 'JJ Social Stream'
    _order = 'sequence asc, id asc'

    name = fields.Char("Title", translate=True)
    media_id = fields.Many2one('jj.social.media', string="Social Media", required=True)
    ...
    account_id = fields.Many2one('jj.social.account', 'Social Account', required=True, ondelete='cascade')
    stream_type_id = fields.Many2one('jj.social.stream.type', string="Type", required=True, ondelete='cascade')
    stream_type_type = fields.Char(related='stream_type_id.stream_type')
    stream_post_ids = fields.One2many('jj.social.stream.post', 'stream_id', 'Posts')
    ...
    def _onchange_media_id(self):
        for stream in self:
            ...
            stream_type_ids = self.env['jj.social.stream.type'].search([('media_id', '=', stream.media_id.id)], limit=2)
    ...
    def refresh_all(self):
        ...
        new_content = False
        for stream in self.env['jj.social.stream'].search([]):
