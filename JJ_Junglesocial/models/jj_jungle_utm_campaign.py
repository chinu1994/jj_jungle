...
class UtmCampaign(models.Model):
    ...
    social_post_ids = fields.One2many('jj.social.post', 'utm_campaign_id', string="All related social media posts", groups="JJ_Junglesocial.group_social_user")
    social_posts_count = fields.Integer(compute="_compute_social_posts_count", string='Social Media Posts', groups="JJ_Junglesocial.group_social_user")
    social_engagement = fields.Integer(compute="_compute_social_engagement", string='Number of interactions (likes, shares, comments ...) with the social posts', groups="JJ_Junglesocial.group_social_user")

    def _compute_social_engagement(self):
        campaigns_engagement = {campaign.id: 0 for campaign in self}

        posts_data = self.env['jj.social.post'].search_read(
    ...
    def _compute_social_posts_count(self):
        ...
        post_data = self.env['jj.social.post']._read_group(
            domain,
    ...
    def action_create_new_post(self):
        ...
        action['context'] = {
            ...
            'default_account_ids': self.env['jj.social.account'].search(self._get_social_media_accounts_domain()).ids
        }
        return action
