...
class JJSocialStreamPost(models.Model):
    """ A 'stream' post, as opposed to a regular jj.social.post, references a post that
    actually exists on a jj.social.media external database (a Facebook post, a Tweet, ...).
    ...
    jj.social.stream.posts are used to fill the 'Feed' view that allows users to follow the jj.social.media activity
    based on their interest (a Facebook Page, a Twitter hashtag, ...).
    They are directly created by their related jj.social.stream. """

    _name = 'jj.social.stream.post'
    _description = 'JJ Social Stream Post'
    ...
    author_link = fields.Char('Author Link', compute='_compute_author_link',
        help="Author link to the external jj.social.media (ex: link to the Twitter Account).")
    post_link = fields.Char('Post Link', compute='_compute_post_link',
        help="Post link to the external jj.social.media (ex: link to the actual Facebook Post).")
    stream_id = fields.Many2one('jj.social.stream', string="Social Stream", ondelete="cascade")
    ...
    stream_post_image_ids = fields.One2many('jj.social.stream.post.image', 'stream_post_id', string="Stream Post Images",
    ...
    # Some jj.social.medias (ex: Facebook) provide information on the link shared with the post.
    ...
    def _fetch_matching_post(self):
        ...
        It returns the jj.social.post linked to this jj.social.stream.post if any, by matching
        the social media specific ID of the jj.social.stream.post to its jj.social.live.post counterpart.
        ...
        self.ensure_one()
        return self.env['jj.social.post']
