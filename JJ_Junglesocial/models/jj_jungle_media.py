...
class JJSocialMedia(models.Model):
    """ A jj.social.media represents the actual Media, ex: Facebook, Twitter, etc...
    As opposed to jj.social.account that represents an existing account on this media.
    Ex: JJ Jungle Social Facebook Page, Mitchell Admin Twitter Account, ...

    The jj.social.media is used to store global media configuration (API keys, ...).
    It's also used to install the modules related to that social media (social_facebook, social_twitter, ...). """

    _name = 'jj.social.media'
    _description = 'JJ Social Media'
    ...
    stream_type_ids = fields.One2many('jj.social.stream.type', 'media_id', string="Stream Types")
